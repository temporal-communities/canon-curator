...
class WikidataClient:
	...
	def _fetch_target(claim: pywikibot.Claim, lang: str) -> dict:
		...
		if hasattr(claim_item, "labels") and hasattr(claim_item, "getID"):
			claim_label = claim_item.labels.get(
				lang, None
			)  # can this ever return a list instead of string?
			claim_id = claim_item.getID()
			return {"type": "item", "label": claim_label, "entity_id": claim_id}
		...
		# isinstance(claim_item, pywikibot.WbMonolingualText)
			...
			return {"type": "literal", "value": claim_item.text}
	...
	def fetch_property(
		self, entity_id: str, property_id: str, lang: str = "en", positive_ranks_only: bool = True
	) -> dict:
		"""
		...
		if not claims:
			return {"entity": entity_id, "property": property_id, "lang": lang, "claims": []}
		...
		for claim in claims:
			...
			claim_data = {**claim_target, "sources": self._fetch_sources(claim), "rank": claim.rank}
